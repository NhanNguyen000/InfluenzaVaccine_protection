## CIT algorithm (R script) from Xun (maybe receive from the author)
########################################################################
...
#--------------------------------------
library(cit) # lastest package from the author

# https://academic.oup.com/bioinformatics/article/32/15/2364/1743433?login=true , the article
# load input data -------------------------
...
inputDat_protein <- protein_normDat %>% 
  ...
  column_to_rownames("Assay") %>% t() %>% as.data.frame 
...
### merge iMED and ZirFlu data-------------------------
overlapped_metabolites <- intersect(iMED_metabolites$Formula, ZirFlu_metabolites$Formula)

inputDat_metabolites <- iMED_metaboliteDat[, overlapped_metabolites] %>% 
  rownames_to_column("name") %>%
  full_join(ZirFlu_metaboliteDat[, overlapped_metabolites] %>% 
              rownames_to_column("name")) %>%
  filter(name %in% metadat_healthy$name) %>%
  column_to_rownames("name")

inputDat_test <- metadat_healthy %>% 
  full_join(inputDat_protein %>% select("CD83") %>% rownames_to_column("name")) %>%
  full_join(inputDat_metabolites %>% select("C4H8O3") %>% rownames_to_column("name")) %>%
  drop_na(CD83, C4H8O3)

# using latest pacakge
# cit.bp(inputDat_test$CD83, inputDat_test$C4H8O3, inputDat_test$H1N1_abFC_combine) # binary outcome
cit.cp(L = inputDat_test$CD83, G = inputDat_test$C4H8O3, T = inputDat_test$H1N1_abFC_combine) # continuous outcome

# using Xun code
CausalityTestJM(LL = as.integer(inputDat_test$CD83), 
                GG = as.matrix(inputDat_test$C4H8O3), 
                TT = as.matrix(inputDat_test$H1N1_abFC_combine))
