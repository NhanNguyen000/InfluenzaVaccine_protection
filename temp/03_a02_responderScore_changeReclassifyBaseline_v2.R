...
strain <- "B"
res.venn2 <- list(
  "LLvsLH" = get.proteinName(resSigPro[[strain]]$LLvsLH, cohorts_dat$proteinAnnot_all),
  "LLvsHL" = get.proteinName(resSigPro[[strain]]$LLvsHL, cohorts_dat$proteinAnnot_all),
  "LLvsHH" = get.proteinName(resSigPro[[strain]]$LLvsHH, cohorts_dat$proteinAnnot_all))
v.table <- venn(res.venn2)


a <- unique(unlist(resSigPro$H1N1)) # 47 proteins
b <- unique(unlist(resSigPro$B)) # 54 proteins
proPverlap_H1N1_B <- intersect(a, b) # 16 proteins
...
picked_proteins <- unique(unlist(resSigPro$H1N1))

picked_proteins <- unique(unlist(c(resSigPro$H1N1, resSigPro$H3N2, resSigPro$B)))
