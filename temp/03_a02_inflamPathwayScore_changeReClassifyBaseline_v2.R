...
# load inflammation score - using average (NES values of inflammation hallmark sets from GSEA) at baseline  -----------------------
...
# hallmarkSubsets_v2
picked_hallmarks <- hallmarkSubsets_v2 # pan-vaccine paper
picked_hallmarks <- hallmarkSubsets_v1
...
ggboxplot(metadat_boxplot, x = "group", y = score,
  ...
  stat_compare_means(comparisons = list(c("young", "old"), c("young", "cirrhosis"), c("old", "cirrhosis")),
                     method = "t.test")
...
ggboxplot(metadat_boxplot, x = "category", y = score,
          paletter = "jco", add = "jitter") + facet_wrap(~group) +
  stat_compare_means(label.y = 2.3) + 
...
# scatter plot
metadat_boxplot %>%
  ggplot(aes(x = H1N1_abFC_combine, y = inflamScore, color = group)) + 
  geom_jitter() +
...
metadat_boxplot %>%
  ggplot(aes(x = H3N2_abFC_combine, y = inflamScore, color = group)) + 
  geom_jitter() +
  geom_smooth(method = lm, se = FALSE) + 
  stat_cor(label.x = 9) + 
  theme_classic() + theme(legend.position = "top")
