...
proteinDat <- protein_Dat$iMED_2015 %>% t()
...
metadat <- cohorts$HAI_all %>% 
  ...
  arrange(match(name, colnames(proteinDat))) 
...
for (strain in strains) {
  ...
  corfit <- duplicateCorrelation(proteinDat,modTime,block = metaFileMerged$probandID)
  print(paste0(strain, " - corfit$consensus.correlation ", corfit$consensus.correlation))
  
  # per timepoint ------------------------------------
  fit_perTime <- lmFit(proteinDat, modTime, 
