...
# iMED
iMED_meboAnnot_temp <- rawDat_iMED_meboAnnot %>% filter(abs(mzDelta) < 0.0001) %>%
  slice(which(Formula %in% hmdb_endogenous$CHEMICAL_FORMULA))
length(unique(iMED_meboAnnot_temp$ionIdx)) # 60 metabolites ionIdx
length(unique(iMED_meboAnnot_temp$Formula)) # 60 metabolites formula
...
# ZirFlu
ZirFlu_meboAnnot_temp <- rawDat_ZirFlu_meboAnnot %>% filter(abs(MzDelta) < 0.0001) %>%
  slice(which(Formula %in% hmdb_endogenous$CHEMICAL_FORMULA))
length(unique(ZirFlu_meboAnnot_temp$ionIdx)) # 473 metabolites
length(unique(ZirFlu_meboAnnot_temp$Formula)) # 475 metabolites formula (1 ionIdx - multiple formulas)
...
# save data ----------------------------------------------------
length(intersect(names(iMED_mebo), names(ZirFlu_mebo)))
mebo_filterMzDelta <- unique(intersect(names(iMED_mebo), names(ZirFlu_mebo)))

save(mebo_filterMzDelta, file = "mebo_filterMzDelta.RData")
