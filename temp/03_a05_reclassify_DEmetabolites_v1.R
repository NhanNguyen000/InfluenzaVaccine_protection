...
DEs <- res.venn$H1N1[-which(res.venn$H1N1 %in% c(res.venn$H3N2, res.venn$B))] # DEs only in H1N1
DEs <- res.venn$B[-which(res.venn$B %in% c(res.venn$H3N2, res.venn$H1N1))] # DEs only in B
DEs <- res.venn$H3N2[-which(res.venn$H3N2 %in% c(res.venn$B, res.venn$H1N1))] # DEs only in H3N2
...
plotDat_order_v3 <- get.plotDat_clusterRow(plotDat_order_v2, colName = "strain", valColumn = "tstatistic")
plotDat_order_v3 %>%
  ggplot(aes(x = strain, y = valName, fill = tstatistic)) + 
  geom_tile() +
  geom_text(aes(label = ifelse(is.na(sig.p.value), NA, "*"))) +
...
plotDat_order_v2 <- plotDat_order %>%
  ...
  mutate(compare = factor(compare, 
                         levels = c("B_LLvsLH", "B_LLvsHL", "B_LLvsHH",
                                    ...
                                    "H1N1_ZirFlu_LLvsLH", "H1N1_ZirFlu_LLvsHL", "H1N1_ZirFlu_LLvsHH")))
