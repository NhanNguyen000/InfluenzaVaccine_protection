...
designTable <- model.matrix(~ + sex + age + H1N1_reclassify, metadat_healthy)
res <- lmFit(inputDat, design = designTable) %>% eBayes() 

# check result
View(res$p.value)
H1N1_reclassify_LLvsLH <-res$p.value %>% as.data.frame() %>% 
  select(H1N1_reclassifyLH) %>% filter(H1N1_reclassifyLH < 0.05)

H1N1_reclassify_LLvsHL <-res$p.value %>% as.data.frame() %>% 
  select(H1N1_reclassifyHL) %>% filter(H1N1_reclassifyHL < 0.05)

H1N1_reclassify_LLvsHH <-res$p.value %>% as.data.frame() %>% 
  select(H1N1_reclassifyHH) %>% filter(H1N1_reclassifyHH < 0.05)

library(ggvenn)
res.venn <- list("LLvsLH" = rownames(H1N1_reclassify_LLvsLH),
             "LLvsHL" = rownames(H1N1_reclassify_LLvsHL),
             "LLvsHH" = rownames(H1N1_reclassify_LLvsHH))
ggvenn(res.venn,  fill_color = c("#999999", "#E69F00", "#56B4E9"))


library(gplots)
res.venn2 <- list(
  "LLvsLH" = get.proteinName(rownames(H1N1_reclassify_LLvsLH), cohorts_dat$proteinAnnot_all),
  "LLvsHL" = get.proteinName(rownames(H1N1_reclassify_LLvsHL), cohorts_dat$proteinAnnot_all),
  "LLvsHH" = get.proteinName(rownames(H1N1_reclassify_LLvsHH), cohorts_dat$proteinAnnot_all))
