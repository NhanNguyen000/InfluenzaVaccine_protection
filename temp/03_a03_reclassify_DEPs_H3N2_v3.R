...
metadat_temp <- metadat_healthy %>% 
  filter(H3N2_reclassify %in% c("LL", "LH", "HH")) %>%
  mutate(H3N2_reclassify_LLvsResponder = ifelse(H3N2_reclassify == "LL", "LL", "responder"))

inputDat_temp <- inputDat[, metadat_temp$name]
identical(metadat_temp$name, colnames(inputDat_temp))

res <- lmFit(inputDat_temp, 
             design =  model.matrix(~ + sex + age + H3N2_reclassify_LLvsResponder, metadat_temp)) %>% 
  eBayes() 

resSig <- res$p.value %>% as.data.frame() %>%  select(4) %>% filter(. <0.05) 
# top.table <- topTable(res, coef= "H3N2_reclassify_LLvsResponderresponder", number = 21) # check again
...
volcanoplot(res, coef= "H3N2_reclassify_LLvsResponderresponder", names = rownames(res), 
            highlight = 5)
...
heatmap_temp <- metadat_heatmap %>% select(name, group, H3N2_reclassify) %>%
  left_join(inputDat_heatmap %>% rownames_to_column("name")) %>%
  group_by(group, H3N2_reclassify) %>%
  summarise_at(vars(-name), funs(mean(., na.rm = TRUE))) %>%
  mutate(name2 = paste0(group, "_", H3N2_reclassify)) %>% 
  column_to_rownames("name2") %>%
  select(resSigPro) %>% t()

heatmap_temp <- metadat_heatmap %>% select(name, group, H3N2_reclassify) %>%
  left_join(inputDat_heatmap %>% rownames_to_column("name")) %>%
  mutate(H3N2_reclassify = ifelse(H3N2_reclassify == "LL", "LL", "LL_andHH")) %>%
  group_by(group, H3N2_reclassify) %>%
  summarise_at(vars(-name), funs(mean(., na.rm = TRUE))) %>%
  mutate(name2 = paste0(group, "_", H3N2_reclassify)) %>% 
  column_to_rownames("name2") %>%
