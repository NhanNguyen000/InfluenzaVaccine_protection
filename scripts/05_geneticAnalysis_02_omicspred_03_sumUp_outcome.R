...
models_mebo_formula <- models_Tab  %>% 
  dplyr::select(model, name, CHEMICAL_FORMULA) %>% drop_na() %>% add_count(CHEMICAL_FORMULA)

models_mebo_formula %>% count(model)
models_mebo_formula %>% filter(n > 1)

models_Tab %>% count(model)
rm(hmdb_endogenous, models_info, models_mebo_formula, models_Tab_temp)

# model predicted outcomes (infomation, predicted value, SNP used) ======================================================
...
# information of the prediction model (.log) ----------------------------------------
...
# Note: extract the variants (SNP) used information from models'log file
pred_logInfo <- pred_logTab[19:20, ] %>% 
  t() %>% as.data.frame() %>% 
...
# Note: model prediction information 
models_pred_Tab <- models_Tab  %>% 
...
rm(pred_logInfo, pred_logTab, fileName, filesList, model_name, models)

# select models to further analysis ======================================================
...
save(selected_models, pred_Tab, file = "processedDat/omicsPred_output.RData")

# plots for OmicPred models ======================================================
models_count <- models_Tab %>% full_join(models_pred_Tab) %>%
...
modelsCount_plot <- models_count %>% 
  ggplot(aes(x = model, y = num_models, fill = status, label = num_models)) + 
  geom_bar(stat = "identity") +
  geom_text(size = 6, position = position_stack(vjust = 0.5)) +
  scale_y_log10(limits = c(1, 1e8), breaks = c(1,1e1, 1e2, 1e3, 1e4, 1e5, 1e6, 1e7, 1e8)) + 
  annotation_logticks(sides = "l")  + 
  scale_fill_manual(values=c( c("#C7C2A6", "#898366"))) +
  theme_classic()+
...
selectedModels_plot <- models_pred_Tab %>% 
  ggplot(aes(x = X.SNP, y = as.numeric(Internal_R2))) + 
  geom_jitter(aes(color = 
                    factor(ifelse(X.SNP >5, model, "not used"))), 
              width = 0.1, size = 3, alpha = 0.5) + 
  scale_color_manual(values = c("grey", "#B65008", "#751915", "#65771E", "#9B2727", "#036879", "#898366"),
                     breaks = c("not used", "RNAseq", "Olink", "Somalogic", "Metabolon", "Nightigale"),
                     labels = c("not used", "RNAseq", "Olink", "Somalogic", "Metabolon", "Nightigale")) + # Adjust colors as needed
  labs(color = "Model") +
  scale_x_log10() +
  annotation_logticks(sides = "b")  + 
  geom_vline(xintercept = 5, linetype = "dashed", color = "black") +
