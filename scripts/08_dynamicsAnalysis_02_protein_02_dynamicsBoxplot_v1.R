...
stat.test <- metadat_boxplot2 %>% 
  group_by(season, H1N1_reclassify) %>%  
  #t_test(TNFRSF13B ~ time, ref.group = "T1") %>% 
  #t_test(IFNLR1 ~ time, ref.group = "T1") %>% 
  #t_test(IL15 ~ time, ref.group = "T1") %>% 
  t_test(CD83 ~ time, ref.group = "T1") %>% 
  add_xy_position() %>% add_significance() %>%
  mutate(xmin = ifelse(H1N1_reclassify == "LL", 0.9, 
                       ifelse(H1N1_reclassify == "LH", 1.9,
                              ifelse(H1N1_reclassify == "HL", 2.9, 3.9))),
...
stat.test_manual  <- stat.test %>% filter(p < 0.05) %>%
  mutate(xmin = c(1.7, 1.7), xmax = c(1.9, 1.9) #, y.position = c(2, 2.5)
  )
stat.test_manual  <- stat.test %>% filter(p < 0.05) %>%
  mutate(xmin = c(1.7, 3.7), xmax = c(1.9, 3.9) , y.position = c(1.7, 2)
  )
