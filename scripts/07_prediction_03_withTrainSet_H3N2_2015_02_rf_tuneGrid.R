...
## input variable ----------------------------------------------------------------
load("processedDat/predictInput_H3N2trainSet.RData")
inputVars <- c("age", "sex", "ab_d0", proNames, meboNames)

# prepare input and validation sets
train_inputSet <- trainSet[, inputVars]
train_predOut <- trainSet[, c("reclassify")] %>% as.vector() %>% unlist() %>% as.factor()
...
selected_method <- "rf" # Random Forest (rf)
predictOutcome <- list()
...
# save the result -----------------------
save(predictModel, predictOutcome, predict_ROC, file = "processedDat/predictOutput_H3N2trainSet_rf_tuneGrid.RData")
