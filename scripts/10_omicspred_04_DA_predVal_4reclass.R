...
a <- p.adjust((res_omicsPred$H1N1_reclassify$p.value %>% as.data.frame())$reclassifyLH, method = "fdr")
range(a)

a <- p.adjust((res_omicsPred$H3N2_reclassify$p.value %>% as.data.frame())$reclassifyHH, method = "fdr")
range(a)
which(a<0.05)

res_omicsPred_fdr <- res_omicsPred %>% 
  lapply(function(x) x$p.value %>% as.data.frame %>% 
           mutate(padj.reclassifyLH = p.adjust(reclassifyLH, method = "fdr"),
                  padj.reclassifyHL = p.adjust(reclassifyHL, method = "fdr"),
                  padj.reclassifyHH = p.adjust(reclassifyHH, method = "fdr"))) 
# sig. for OPGS009766 - TEN2 (H3N2 - LH, HL, HH), OPGS009008 - USP46 (H3N2 - LH, HH), OPGS005001 - RAB44 s(B - LH)

# check venn diagram 
res.venn <- DAs %>% lapply(function(x) x %>% lapply(function(y) rownames(y)))
v.table <- venn(res.venn$H1N1_reclassify)
v.table <- venn(res.venn$H3N2_reclassify)
v.table <- venn(res.venn$B_reclassify)

padj <- res_omicsPred %>% 
  lapply(function(x) x %>% 
           eBayes %>% topTable(n = Inf, sort.by = "n") %>% 
           filter(adj.P.Val < 0.05)) # get sig. for OPGS009766 model
...
DAs_all <- DAs %>% 
  lapply(function(x) 
    x %>% 
      lapply(function(y) y %>% rownames_to_column("valName")) %>% purrr::reduce(full_join)) %>%
  bind_rows(.id = "strain") %>%
...
selected_DAs <- c(intersect(intersect(res.venn$H1N1_reclassify$LLvsLH, 
                                    res.venn$H1N1_reclassify$LLvsHL), 
                          res.venn$H1N1_reclassify$LLvsHH),
                  intersect(intersect(res.venn$H3N2_reclassify$LLvsLH, 
                                      res.venn$H3N2_reclassify$LLvsHL), 
                            res.venn$H3N2_reclassify$LLvsHH),
                  intersect(intersect(res.venn$B_reclassify$LLvsLH, 
                                      res.venn$B_reclassify$LLvsHL), 
                            res.venn$B_reclassify$LLvsHH)) %>% unique
...
selected_DAs_info_v2.1 <- selected_DAs_info %>% #as.data.frame() %>% 
...
test <- tstat_longDat %>% filter(valName %in% selected_DAs) %>%
