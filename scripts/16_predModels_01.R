...
for (selected_method in method_list) {
  ...
  for (i in 1:nreps) {
    ...
    for (valiSet in names(validation_inputSets)) {
      val.pred_temp <- predict(netFits[[selected_method]][[i]], newdata = validation_inputSets[[valiSet]], type = 'raw')
      netFit_pred[[selected_method]][[valiSet]][[i]] <- confusionMatrix(val.pred_temp, 
                                                                        validation_predOuts[[valiSet]]) # Training accuracy
    }
...
# save the result -----------------------
save(netFits, netFit_pred, file = "res.predModels.RData")
