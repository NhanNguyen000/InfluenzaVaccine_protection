...
H1N1_MN_2015 <- metadata_healthy %>% filter(season == "2015") %>%
  ...
  inner_join(cohorts$MN_all %>% select(probandID, H1N1_T1_log2, H1N1_T4_log2, H1N1_abFC)) %>%
...
plotDat_NRvsR <- H1N1_MN_2015  %>%
  ggplot(aes(time, MNtiter)) + geom_boxplot() + geom_point() +
...
# other boxplot compare T1 vs T4 per 4 classes: LL, LH, HL, HH
