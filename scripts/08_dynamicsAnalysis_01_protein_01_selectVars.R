...
selected_vars <- unique(plotDat_DAPs$valName %>% as.vector())
...
length(unique(sigOutcome$var)) 
sigVar_dynamic <- unique(sigOutcome$var)

save(outcome, sigOutcome, sigVar_dynamic, file = "sigProtein_dynamic_from35selectedProteins.RData")
