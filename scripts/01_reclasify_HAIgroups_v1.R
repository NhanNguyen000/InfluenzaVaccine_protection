...
plot_list <- list()
for (year in seasons) {
  HAI_reclassify_temp <- ZirFlu_HAIreclassify  %>% filter(season == year)
...
HAIreclassify$ZirFlu <- ZirFlu_HAIreclassify
HAIreclassify$iMED <- iMED_HAIreclassify

HAIreclassify$all_cohorts <- HAIreclassify %>% purrr::reduce(full_join) %>%
...
HAIreclassify$all_cohorts %>% count(sameResponse_2Bstrains)

save(HAIreclassify, file = "HAIreclassify.RData")


# check iMED --------------
iMED$HAItiter %>% count(H1N1_T1 >= log2(20))
iMED$HAItiter %>% count(ab_H1N1 >= 4)
