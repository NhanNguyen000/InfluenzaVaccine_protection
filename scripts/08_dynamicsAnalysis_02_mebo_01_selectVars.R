...
metabols <- mebo_Dat %>% 
  ...
  column_to_rownames("name") %>% select(selected_vars) %>%
  t() %>% as.data.frame %>% select(metadat$name)
...
for (group in groups) {
  ...
  corfit <- duplicateCorrelation(metabols_temp, modTime, block = metaFileMerged$probandID)
...
## strain with season have only 3 groups: LL, LH, HL-----------------------------------
groups <- c("2020_H1N1")
...
sigVar_dynamic <- unique(sigOutcome$var)

save(outcome, sigOutcome, sigVar_dynamic, file = "sigMebo_dynamic_from30selectedMebo.RData")

a <- sigOutcome %>% add_count(var)
# metabolite, HMDB ids -----------------------------------
# library(openxlsx)
...
# 
# k <- rawDat_iMED_meboAnnot %>% filter(Formula %in% unique(sigOutcome$var)) # 344 formulas
# k2 <- k %>% slice(grep("HMDB", k$CompoundID))
# write.table(k2$CompoundID, file = "processedDat/sigMeboDynamic_HMDB.txt", quote = FALSE, col.names = FALSE, row.names = FALSE)
# 
# k <- rawDat_iMED_meboAnnot %>% filter(Formula %in% names(mebo_Dat$iMED_2014)) # 508 formulas
# k2 <- k %>% slice(grep("HMDB", k$CompoundID))
# write.table(k2$CompoundID, file = "processedDat/allMeboUsed_HMDB.txt", quote = FALSE, col.names = FALSE, row.names = FALSE)
# 

# check the overlap -----------------------------------
a <- sigOutcome %>% group_by(var) %>% add_count()
range(a$n)
a2 <- a %>% filter(n == 28)
unique(a2$var)
"C5H11NO2S" # metholine
"C19H36O2"   "C19H26N2O2" "C15H25NO4"  "C18H30O2"   "C17H32O3" 

b1 <- sigOutcome %>% filter(compare %in% c("HH_d28vsd0", "HH_d7vsd0")) %>%
  group_by(var) %>% add_count()
unique((b1 %>% filter(n == 8))$var)
"C3H7NO2"   "C5H11NO2S"

b2 <- sigOutcome %>% filter(compare %in% c("LH_d28vsd0", "LH_d7vsd0")) %>%
  group_by(var) %>% add_count()
unique((b2 %>% filter(n >= 6))$var)

b3 <- sigOutcome %>% filter(compare %in% c("HL_d28vsd0", "HL_d7vsd0")) %>%
  group_by(var) %>% add_count()
unique((b3 %>% filter(n == 8))$var)

b4 <- sigOutcome %>% filter(compare %in% c("LL_d28vsd0", "LL_d7vsd0")) %>%
  group_by(var) %>% add_count()
unique((b4 %>% filter(n == 8))$var)

a <- sigOutcome %>% 
  filter(group %in% c("2015_H1N1", "2015_H3N2", "2015_B"),
         compare %in% c("LL_d28vsd0", "LL_d7vsd0")) %>%
  group_by(var) %>% add_count()
unique((a %>% filter(n >= 6))$var)
